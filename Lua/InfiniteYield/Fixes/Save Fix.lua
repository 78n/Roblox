...
function saves()
	...
	if type(writefile) == "function" and type(readfile) == "function" then
        local suc,out = pcall(readfile, "IY_FE.iy")

		if suc then
			if out == nil or xpcall(function()
					local Settings = HttpService:JSONDecode(out)
                    if Settings.prefix ~= nil then prefix = Settings.prefix else prefix = ';' end
                    if Settings.StayOpen ~= nil then StayOpen = Settings.StayOpen else StayOpen = false end
                    if Settings.keepIY ~= nil then KeepInfYield = Settings.keepIY else KeepInfYield = true end
                    if Settings.espTransparency ~= nil then espTransparency = Settings.espTransparency else espTransparency = 0.3 end
                    if Settings.logsEnabled ~= nil then logsEnabled = Settings.logsEnabled else logsEnabled = false end
                    if Settings.jLogsEnabled ~= nil then jLogsEnabled = Settings.jLogsEnabled else jLogsEnabled = false end
                    if Settings.aliases ~= nil then aliases = Settings.aliases else aliases = {} end
                    if Settings.binds ~= nil then binds = (Settings.binds or {}) else binds = {} end
                    if Settings.spawnCmds ~= nil then spawnCmds = Settings.spawnCmds end
                    if Settings.WayPoints ~= nil then AllWaypoints = Settings.WayPoints else WayPoints = {} AllWaypoints = {} end
                    if Settings.PluginsTable ~= nil then PluginsTable = Settings.PluginsTable else PluginsTable = {} end
                    if Settings.currentShade1 ~= nil then currentShade1 = Color3.new(Settings.currentShade1[1],Settings.currentShade1[2],Settings.currentShade1[3]) end
                    if Settings.currentShade2 ~= nil then currentShade2 = Color3.new(Settings.currentShade2[1],Settings.currentShade2[2],Settings.currentShade2[3]) end
                    if Settings.currentShade3 ~= nil then currentShade3 = Color3.new(Settings.currentShade3[1],Settings.currentShade3[2],Settings.currentShade3[3]) end
                    if Settings.currentText1 ~= nil then currentText1 = Color3.new(Settings.currentText1[1],Settings.currentText1[2],Settings.currentText1[3]) end
                    if Settings.currentText2 ~= nil then currentText2 = Color3.new(Settings.currentText2[1],Settings.currentText2[2],Settings.currentText2[3]) end
                    if Settings.currentScroll ~= nil then currentScroll = Color3.new(Settings.currentScroll[1],Settings.currentScroll[2],Settings.currentScroll[3]) end
                    if Settings.eventBinds ~= nil then loadedEventData = Settings.eventBinds end
				end, function(err)
                    local suc,out = pcall(readfile, "IY_FE.iy")

                    if suc then
                        pcall(writefile, "IY_FE_BACKUP.iy", out)
                    end
                    Popup("An error occured when attemping to extract the json from IY_FE.iy\nYour data has been overwritten with the default configs"..(suc and "and a backup of your old data can be found in IY_FE_BACKUP.iy" or ". Your old data could not be backed up.").."\nError:\n"..err)
                end) then
                useFactorySettings()
				pcall(writefile,"IY_FE.iy", defaults)
			end
		else
			pcall(writefile,"IY_FE.iy", defaults)
			wait()
            useFactorySettings()
			if not pcall(readfile,"IY_FE.iy") then
				nosaves = true
                Popup("There was a problem reading/writing the save file (IY_FE.iy).\n\nPlease contact the developer/support team for your exploit and tell them writefile/readfile is not working.\n\nYour settings, keybinds, waypoints, and aliases will not save if you continue.\n\nThings to try:\n> Make sure a 'workspace' folder is located in the same folder as your exploit\n> If your exploit is inside of a zip/rar file, extract it.\n> Rejoin the game and try again or restart your PC and try again.")
			end
		end
	else
        nosaves = true
        useFactorySettings()
        Popup("Sorry, but we are unable to save your data due to your exploit missing file functions (readfile and writefile).\n\nInfinite Yield is and will be using factory default settings until your executor adds support for these functions (readfile and writefile)")
	end
end
